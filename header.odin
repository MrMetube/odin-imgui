...
draw_list_add_image                      :: #force_inline proc(self: ^Draw_List, user_texture_id: Texture_ID, p_min: Vec2, p_max: Vec2, uv_min := Vec2 {0,0}, uv_max := Vec2 {1,1}, col := u32(4294967295))  { ImDrawList_AddImage(self, user_texture_id, p_min, p_max, uv_min, uv_max, col);}
draw_list_add_image_quad                 :: #force_inline proc(self: ^Draw_List, user_texture_id: Texture_ID, p1: Vec2, p2: Vec2, p3: Vec2, p4: Vec2, uv1 := Vec2 {0,0}, uv2 := Vec2 {1,0}, uv3 := Vec2 {1,1}, uv4 := Vec2 {0,1}, col := u32(4294967295)) { ImDrawList_AddImageQuad(self, user_texture_id, p1, p2, p3, p4, uv1, uv2, uv3, uv4, col);}
...
font_atlas_add_custom_rect_font_glyph                 :: #force_inline proc(self: ^Font_Atlas, font: ^ImFont, id: Wchar, width: i32, height: i32, advance_x: f32, offset := Vec2 {0,0}) -> i32                               { return ImFontAtlas_AddCustomRectFontGlyph(self, font, id, width, height, advance_x, offset);}
...
begin_child_str                        :: #force_inline proc(str_id: string, size := Vec2 {0,0}, border := bool(false), flags := Window_Flags(0)) -> bool                                                                     { return swr_igBeginChild_Str(str_id, size, border, flags);}
begin_child_id                         :: #force_inline proc(id: ImID, size := Vec2 {0,0}, border := bool(false), flags := Window_Flags(0)) -> bool                                                                           { return igBeginChild_ID(id, size, border, flags);}
...
begin_list_box                         :: #force_inline proc(label: string, size := Vec2 {0,0}) -> bool                                                                                                                       { return swr_igBeginListBox(label, size);}
...
begin_table                            :: #force_inline proc(str_id: string, column: i32, flags := Table_Flags(0), outer_size := Vec2 {0.0,0.0}, inner_width := f32(0.0)) -> bool                                             { return swr_igBeginTable(str_id, column, flags, outer_size, inner_width);}
...
button                                 :: #force_inline proc(label: string, size := Vec2 {0,0}) -> bool                                                                                                                       { return swr_igButton(label, size);}
...
color_button                           :: #force_inline proc(desc_id: string, col: Vec4, flags := Color_Edit_Flags(0), size := Vec2 {0,0}) -> bool                                                                            { return swr_igColorButton(desc_id, col, flags, size);}
...
image                                  :: #force_inline proc(user_texture_id: Texture_ID, size: Vec2, uv0 := Vec2 {0,0}, uv1 := Vec2 {1,1}, tint_col := Vec4 {1,1,1,1}, border_col := Vec4 {0,0,0,0})       { igImage(user_texture_id, size, uv0, uv1, tint_col, border_col);}
image_button                           :: #force_inline proc(user_texture_id: Texture_ID, size: Vec2, uv0 := Vec2 {0,0}, uv1 := Vec2 {1,1}, frame_padding := i32(-1), bg_col := Vec4 {0,0,0,0}, tint_col := Vec4 {1,1,1,1}) -> bool { return igImageButton(user_texture_id, size, uv0, uv1, frame_padding, bg_col, tint_col);}
...
input_text_multiline                   :: #force_inline proc(label: string, buf: string, buf_size: uint, size := Vec2 {0,0}, flags := Input_Text_Flags(0), callback : Input_Text_Callback = nil, user_data : rawptr = nil) -> bool { return swr_igInputTextMultiline(label, buf, buf_size, size, flags, callback, user_data);}
...
plot_histogram_float_ptr               :: #force_inline proc(label: string, values: ^f32, values_count: i32, values_offset := i32(0), overlay_text := "", scale_min := f32(max(f32)), scale_max := f32(max(f32)), graph_size := Vec2 {0,0}, stride := i32(size_of(f32))) { swr_igPlotHistogram_FloatPtr(label, values, values_count, values_offset, overlay_text, scale_min, scale_max, graph_size, stride);}
...
plot_lines_float_ptr                   :: #force_inline proc(label: string, values: ^f32, values_count: i32, values_offset := i32(0), overlay_text := "", scale_min := f32(max(f32)), scale_max := f32(max(f32)), graph_size := Vec2 {0,0}, stride := i32(size_of(f32))) { swr_igPlotLines_FloatPtr(label, values, values_count, values_offset, overlay_text, scale_min, scale_max, graph_size, stride);}
...
progress_bar                           :: #force_inline proc(fraction: f32, size_arg := Vec2 {-min(f32),0}, overlay := "")                                                                                                    { swr_igProgressBar(fraction, size_arg, overlay);}
...
selectable_bool                        :: #force_inline proc(label: string, selected := bool(false), flags := Selectable_Flags(0), size := Vec2 {0,0}) -> bool                                                                { return swr_igSelectable_Bool(label, selected, flags, size);}
selectable_bool_ptr                    :: #force_inline proc(label: string, p_selected: ^bool, flags := Selectable_Flags(0), size := Vec2 {0,0}) -> bool                                                                      { return swr_igSelectable_BoolPtr(label, p_selected, flags, size);}
...
set_next_window_pos                    :: #force_inline proc(pos: Vec2, cond := Cond(0), pivot := Vec2 {0,0})                                                                                                                 { igSetNextWindowPos(pos, cond, pivot);}
...
text                                   :: text_formatted
text_formatted                         :: #force_inline proc(fmt_: string, args: ..any)                                                                                                                                             { wrapper_text(fmt_, ..args);}
